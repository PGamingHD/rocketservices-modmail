...
client.on("messageCreate", async (message) => {
    ...
    if (message.channel.type === ChannelType.DM) {
        const [categoryIdRows, categoryIdFields] = await client.connection.query(`SELECT modmail_categoryid FROM modmail_setup`);

        if (categoryIdRows.length !== 1) {
            return message.reply({
                embeds: [
                    new EmbedBuilder()
                    .setColor(embed.errorColor)
                    .setDescription(`:x: Looks like no category has been made in this server yet, please use /setup first!`)
                ]
            })
        }

        const categoryID = categoryIdRows[0].modmail_categoryid;

        const [transOpenRows, transOpenFields] = await client.connection.query(`SELECT * FROM modmail_data WHERE modmail_owner = ${message.author.id} AND modmail_status = 1`)

        if (transOpenRows.length !== 0) {
            const mainGuild = client.guilds.cache.get(config.MODMAIL_GUILD);
            const channel = mainGuild.channels.cache.get(transOpenRows[0].modmail_channelid);
            const category = mainGuild.channels.cache.get(categoryID);
            ...
            if (!category) {
                return message.reply({
                    embeds: [
                        new EmbedBuilder()
                        .setColor(embed.errorColor)
                        .setDescription(`:x: No category was found for this, please use the /setup command!`)
                    ]
                })
            }

            if (!channel) {
                await client.connection.query(`DELETE FROM modmail_data WHERE modmail_owner = ${message.author.id} AND modmail_status = 1`)
                ...
                const createNew = await mainGuild.channels.create({
                ...
                const mainMSG = await createNew.send({
                    embeds: [
                        new EmbedBuilder()
                        .setColor(embed.color)
                        .setTitle(`New Modmail Case`)
                        .setDescription(`*${message.content}*`)
                        .setAuthor({
                            name: `${message.author.username}#${message.author.discriminator}`,
    ...
    if (message.channel.type === ChannelType.GuildText) {
        const [channelIdRows, channelIdFields] = await client.connection.query(`SELECT * FROM modmail_data WHERE modmail_channelid = ${message.channel.id}`);

        if (channelIdRows.length !== 0) {
            const channelData = channelIdRows[0];

            if (!channelData.modmail_status) {
                await message.delete();
                return;
            }

            if (parseInt(channelData.modmail_claimed) === 0) {
                await message.delete();
                const failMSG = await message.channel.send({
                    embeds: [
                        new EmbedBuilder()
                        .setColor(embed.errorColor)
                        .setDescription(`:x: Please claim this case before sending messages in here!`)
                        .setFooter({
                            text: `${message.author.username}#${message.author.discriminator} | ${message.author.id}`
                        })
                        .setTimestamp()
            ...
            if (parseInt(channelData.modmail_claimed) !== parseInt(message.author.id)) {
                await message.delete();
                const failMSG = await message.channel.send({
                    embeds: [
                        new EmbedBuilder()
                        .setColor(embed.errorColor)
                        .setDescription(`:x: You must be owner of this Case to send messages here!`)
                        .setFooter({
                            text: `${message.author.username}#${message.author.discriminator} | ${message.author.id}`
                        })
                        .setTimestamp()
            ...
            const user = await client.users.fetch(`${channelData.modmail_owner}`);
